...
function phaseSortIndex(fase) {
  const idx = KNOCKOUT_PHASE_ORDER.indexOf(fase);
  return idx === -1 ? 99 : idx;
}
...
function MatchRow({ match, onSubmitted }) {
  ...
  const canEdit = match.can_submit && !match.submitted;
  const fechaLabel = formatMatchSchedule(match);
  const highlight = match.is_active_round && canEdit;

  return (
    <div
      className={`rounded-xl border p-4 ${
        highlight
          ? "border-emerald-500/50 bg-emerald-500/5 dark:border-emerald-500/40"
    ...
    >
      ...
      {match.submitted ? (
        ...
        <p className="text-sm text-slate-500">
          {match.estado === "SCHEDULED" || match.estado === "LIVE" || match.estado === "FINISHED"
            ? "No puedes enviar predicción para este partido."
            : "Plazo cerrado para este partido."}
        </p>
      )}
    </div>
  );
...
export default function MisPredicciones() {
  ...
  const { data: roundData, isLoading, refetch } = useRoundMatches();
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">
          ...
          <strong>una sola vez</strong> y no puede editarse después.
        </p>
      </div>
